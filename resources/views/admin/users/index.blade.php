...
@section('title', 'Registry Control - Admin')

@section('header')
    <div class="flex justify-between items-end">
        <h1 class="text-3xl font-black text-black uppercase tracking-tighter">System Registry</h1>
        <div class="flex space-x-4">
            <a href="{{ route('admin.users.export', request()->all()) }}" class="bg-black/5 text-black px-6 py-3 rounded-xl font-black uppercase tracking-widest text-[10px] no-underline hover:bg-black hover:text-brand transition">
                Export Current View
            </a>
            <a href="{{ route('admin.users.create') }}" class="bg-black text-brand px-8 py-3 rounded-xl font-black uppercase tracking-widest text-xs no-underline hover:opacity-80 transition shadow-xl">
                Register New User
            </a>
        </div>
    </div>
...
<div class="space-y-8" x-data="{ showColumns: { email: true, role: true, status: true, joined: true } }">
    ...
    <div class="bg-black/5 p-8 rounded-3xl border border-black/5 shadow-sm">
        <form action="{{ route('admin.users.index') }}" method="GET" id="filter-form" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-6">
                <!-- Global Search -->
                <div class="md:col-span-2">
                    <label class="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-2">Omni Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Name or identifier..."
                           class="w-full bg-white/20 border-black/10 rounded-xl px-4 py-3 text-black font-bold focus:ring-black">
                </div>

                <!-- Status Filter -->
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-2">Status</label>
                    <select name="status" class="w-full bg-white/20 border-black/10 rounded-xl px-4 py-3 text-black font-bold focus:ring-black">
                ...
                <!-- Role Filter -->
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-2">Access Role</label>
                    <select name="role_id" class="w-full bg-white/20 border-black/10 rounded-xl px-4 py-3 text-black font-bold focus:ring-black">
                ...
                <!-- Per Page -->
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-2">Density</label>
                    <select name="per_page" class="w-full bg-white/20 border-black/10 rounded-xl px-4 py-3 text-black font-bold focus:ring-black">
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full bg-black text-brand py-3 rounded-xl font-black uppercase tracking-widest text-[10px] hover:opacity-80 transition">Apply Logic</button>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center pt-6 border-t border-black/5">
                ...
                <div class="flex items-center space-x-4">
                    <span class="text-[10px] font-black uppercase tracking-widest opacity-40">Column Visibility:</span>
                    <template x-for="(visible, col) in showColumns">
                        <button type="button" @click="showColumns[col] = !showColumns[col]"
                                :class="visible ? 'bg-black text-brand' : 'bg-black/5 text-black/40'"
                                class="px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest transition"
                                x-text="col"></button>
                    </template>
                </div>

                <a href="{{ route('admin.users.index') }}" class="text-[10px] font-black uppercase tracking-widest text-black/40 hover:text-black no-underline transition border-b border-black/10">Reset Defaults</a>
            </div>
        </form>
    </div>
