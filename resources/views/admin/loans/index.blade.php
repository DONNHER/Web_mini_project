...
@section('header')
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-black uppercase tracking-tighter leading-none">Loan Management</h1>
        </div>
        <div class="flex space-x-2">
            <a href="{{ route('admin.export') }}" class="btn-secondary px-4 no-underline">
                Export Data
            </a>
...
@section('content')
<div class="space-y-6" x-data="{ showColumns: { identifier: true, principal: true, status: true, date: true } }">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Scan identities or Loan IDs..."
                           class="w-full bg-[#FEF6F0] border-none rounded-lg px-6 py-2 text-black font-bold focus:ring-2 focus:ring-[#FF6B00]/5 placeholder-black/20 text-xs outline-none">
                </div>
    ...
    <!-- Table Container (Users Style) -->
    <div class="card overflow-hidden">
        <div class="px-6 py-4 border-b border-black/5 flex justify-between items-center bg-[#FEF6F0]/30">
            <div class="flex items-center space-x-2">
                <select name="bulk_action" class="bg-white/50 border-black/10 rounded-lg px-4 py-1.5 text-[8px] font-black uppercase tracking-widest focus:ring-black">
                    <option value="">Bulk Actions</option>
                    <option value="approve">Approve</option>
                    <option value="reject">Reject</option>
                </select>
                <button type="button" class="bg-black text-brand px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest hover:opacity-80">Execute</button>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-black text-white uppercase text-[8px] font-black tracking-widest">
                    <tr>
                        <th class="px-6 py-4"><input type="checkbox" class="rounded border-white/20 bg-white/10 text-brand focus:ring-brand"></th>
                        <th class="px-4 py-4">Identity</th>
                        <th x-show="showColumns.identifier" class="px-4 py-4">Identifier</th>
                ...
                <tbody class="divide-y divide-black/5">
                    ...
                    <tr class="group hover:bg-[#FEF6F0] transition-colors">
                        <td class="px-6 py-4"><input type="checkbox" class="rounded border-black/10 text-black"></td>
                        <td class="px-4 py-4">
                        ...
                        <td x-show="showColumns.identifier" class="px-4 py-4 text-[10px] font-bold text-[#FF6B00]">#{{ str_pad($loan->id, 4, '0', STR_PAD_LEFT) }}</td>
                        <td x-show="showColumns.principal" class="px-4 py-4 text-[10px] font-black text-black">₱{{ number_format($loan->principal_amount) }}</td>
                        <td x-show="showColumns.status" class="px-4 py-4">
                            <span class="flex items-center text-[8px] font-black uppercase">
                                <span class="h-1.5 w-1.5 rounded-full mr-1.5
                                    @if($loan->status == 'completed' || $loan->status == 'active') bg-green-500
                                    @elseif($loan->status == 'pending' || $loan->status == 'approved') bg-yellow-500
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('loans.show', $loan) }}" class="text-black font-black text-[8px] uppercase tracking-widest border-b border-black hover:text-[#FF6B00] hover:border-[#FF6B00] no-underline">View Details</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
